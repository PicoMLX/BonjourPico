...
public actor BonjourDiscoveryActor {
    ...
    public func serviceStream() -> AsyncThrowingStream<[BonjourEndpoint], Error> {
        ...
        return AsyncThrowingStream { continuation in
            Task { [weak self] in
                ...
                await self.addEndpointContinuation(continuation, id: id)
            }
    ...
    private func handleStateUpdate(_ state: NWBrowser.State) async {
        browserState = state
        broadcastState()
        switch state {
    ...
    private func scheduleRestart() async {
        restartTask?.cancel()
    ...
    private func performRestartIfNeeded() async {
        guard browser != nil else { return }
        stop()
        do {
            try await startBrowser()
